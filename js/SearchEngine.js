...
(function (MessageType) {
    ...
    MessageType["resultAll"] = "resultAll";
...
var resultFromInfo = function (i) { return ({
    ...
    final: isEvent(i) ? getFinalEndTime(i) : null
}); };
...
var occurrenceAtTime = function (time) { return function (o) { return new Date(o.start_time).valueOf() <= time && new Date(o.end_time).valueOf() > time; }; };
var getFinalEndTime = function (e) { return isEmpty(e.occurrence_set) ? null : new Date(e.occurrence_set[e.occurrence_set.length - 1].end_time); };
//const sortByUniqueness = (a:BMEvent, b:BMEvent) => a.occurrence_set!.length - b.occurrence_set!.length;
var sortByTimeRemaining = function (time) {
    //const occAtTime = occurrenceAtTime(time);
    //const timeRemaining = (e:BMEvent) => new Date(e.occurrence_set!.find(occAtTime)!.end_time).valueOf() - time;
    var timeRemaining = function (e) { return getFinalEndTime(e).valueOf() - time; };
    var sortFunc = function (a, b) { return timeRemaining(a) - timeRemaining(b); };
    return sortFunc;
};
var SearchEngine = /** @class */ (function () {
    function SearchEngine(data, searchLib) {
        ...
        var eventsAtTime = function (time) { return data.events.filter(function (e) { return e.occurrence_set !== undefined && e.occurrence_set.some(occurrenceAtTime(time)); }); };
        ...
        var addEventLocationToFuseResult = function (r) { return assoc('item', addEventLocation(r.item), r); };
        ...
        this.getResultsByTerm = function (term) { return search(term).map(function (r) { return isEvent(r.item) ? addEventLocationToFuseResult(r) : r; }).map(resultFromFuseResult); };
        this.eventResultsAtTime = function (time) { return eventsAtTime(time).slice().sort(sortByTimeRemaining(time)).map(addEventLocation).map(resultFromInfo); };
    }
    return SearchEngine;
}());