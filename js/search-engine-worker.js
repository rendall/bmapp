...
var sendResults = function (results) {
    ...
    if (results.length === 0)
        return;
    var rem = BUFFER_COUNT - results.length % BUFFER_COUNT;
    var fill = Array(rem).fill({});
    var resultsAndFill = results.concat(fill);
    //const allResultSub:Rx.Observable<BMResultItem> =  ;
    var resSubVal = resultStream.subscribe(function (r) { return sendMessage(MessageType.resultAdd, r); });
    resultSubscription.next(resSubVal);
    var restultObs = Rx.Observable.from(resultsAndFill).bufferCount(BUFFER_COUNT);
